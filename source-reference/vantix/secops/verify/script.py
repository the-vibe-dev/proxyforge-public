...
_LOGGER = logging.getLogger("secops.verify.script")
...
class ScriptVerifier(ReplayVerifier):
    ...
    def verify(self, spec: ReplaySpec, ctx: VerifyContext) -> VerifyOutcome:
        ...
        # Soft-fix per repo audit 2026-05-15: ScriptVerifier historically lacked
        # ScopeEgressGuard parity with HttpVerifier + BrowserVerifier. We do not
        # refuse out-of-scope URLs (a hard cut would break existing replays);
        # instead we log a structured warning so operators can see the gap and
        # tighten ad-hoc scripts to scope. Hard refusal is a follow-up.
        guard = ScopeEgressGuard.from_context(ctx.extras) if getattr(ctx, "extras", None) else None
        if guard is not None:
            decision = guard.evaluate(url)
            if not getattr(decision, "allowed", True):
                _LOGGER.warning(
                    "script_verifier_out_of_scope url=%s reason=%s soft_fix=true",
                    url,
                    getattr(decision, "reason", "scope-guard-denied"),
                )
