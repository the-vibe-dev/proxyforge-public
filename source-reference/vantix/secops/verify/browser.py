    ...
    def verify(self, spec: ReplaySpec, ctx: VerifyContext) -> VerifyOutcome:
        ...
        if guard is not None:
            ...
            if not decision.allowed:
                return VerifyOutcome(validated=False, reason=f"scope egress blocked: {decision.reason}", signal={"blocked": True, "url": url})
        ...
        return VerifyOutcome(
            validated=True,
            reproduction_script=f"sechive-browser assess {url}",
            signal=signal,
        )
