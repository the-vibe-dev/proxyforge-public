---
name: ctf_web_challenge_classifier
description: Classifies a web challenge by style BEFORE tool use: auth bypass / SQLi / SSTI / deserialization / JWT / client-side trick / race / SSRF / file upload / path traversal / crypto-web hybrid. Saves time by preventing generic payload spam.
---
# CTF Web Challenge Classifier

Classifies a web challenge by style BEFORE tool use: auth bypass / SQLi / SSTI / deserialization / JWT / client-side trick / race / SSRF / file upload / path traversal / crypto-web hybrid. Saves time by preventing generic payload spam.
...
- Login page + suspicious comment + admin endpoint → auth bypass class (SQLi auth, NULL byte, etc.).
- Template syntax artifacts in response → SSTI (Jinja/Twig/Velocity classifier).
- JWT in request + algorithm:none or weak HS256 → JWT lab.
- Cookie/POST body has known serialization markers (PHP, Python pickle, Java) → deserialization.
- Heavy client-side JS doing the 'real' check → bypass via direct API call.
...
- `ctf_web_challenge_classifier.auth-bypass-classifier`
- `ctf_web_challenge_classifier.ssti-classifier`
- `ctf_web_challenge_classifier.jwt-classifier`
- `ctf_web_challenge_classifier.deserialization-classifier`
- `ctf_web_challenge_classifier.client-side-trick-classifier`
- `ctf_web_challenge_classifier.upload-traversal-classifier`

## Tooling

Optional external tools: curl, a web proxy. The pack also works with vanilla `curl` + manual inspection.
