...
exports.buildFinding = buildFinding;
exports.buildIssueFromFinding = buildIssueFromFinding;
function severityFromFamily(family) {
    if (/command-injection|xxe|deserialization|log4|spring4/i.test(family))
        return 'critical';
    if (/sql-injection|ssrf|lfi-traversal|nosql|xpath|ldap|prototype-pollution/i.test(family))
        return 'high';
    if (/xss|ssti|open-redirect|csrf|jwt|session-fixation|cache-poisoning/i.test(family))
        return 'medium';
    if (/crlf|host-header|csv-formula|clickjacking/i.test(family))
        return 'low';
    return 'medium';
}
function titleFromFamily(family, checkId) {
    const map = {
...
function remediationFromFamily(family) {
    const map = {
...
function buildFinding(checkId, family, host, path, insertionPointId, conclusion, bestClassification, variant, exchangeId) {
    const id = `finding-${checkId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    const confidence = conclusion.confidence >= 0.85 ? 'certain' :
...
function buildIssueFromFinding(finding) {
    return {
        id: `issue-${finding.id}`,
        title: finding.title,
