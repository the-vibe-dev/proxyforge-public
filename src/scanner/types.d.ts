export type PayloadFamily = 'sql-injection' | 'xss-reflected' | 'xss-oracle' | 'ssti' | 'ssti-blind-time' | 'lfi-traversal' | 'command-injection' | 'command-injection-blind-time' | 'command-injection-blind-oast' | 'ssrf' | 'ssrf-oast' | 'open-redirect' | 'crlf-injection' | 'host-header' | 'cache-poisoning' | 'xxe' | 'xxe-oast' | 'nosql-injection' | 'xpath-injection' | 'ldap-injection' | 'expression-language-injection' | 'csv-formula-injection' | 'jwt-attack' | 'deserialization' | 'request-smuggling' | 'prototype-pollution-client' | 'prototype-pollution-server' | 'dom-xss' | 'graphql-attack' | 'mass-assignment' | 'idor' | 'cors-misconfig' | 'clickjacking' | 'csrf' | 'session-fixation' | 'cache-deception' | 'cve-named' | 'csrf-heuristic' | 'postMessage-misconfig';
export type InsertionPointKind = 'query' | 'body' | 'form' | 'header' | 'path' | 'json' | 'xml' | 'graphql' | 'cookie' | 'multipart';
...
export type OracleResponseClass = 'expected-proof' | 'verifier-type-error' | 'wrong-observed-value' | 'observed-value' | 'tag-stripped-or-ignored' | 'neutral-or-not-parsed' | 'reflected-inert' | 'method-or-parser-rejected' | 'parser-error' | 'timing-delta' | 'length-delta' | 'status-delta' | 'oast-callback-confirmed' | 'unknown';
